...
class SolverTest < Minitest::Test
  ...
  def test_solve_one_missing_number
    ...
    assert_equal [2], solver.find_remaining_num(input)
  end

  def test_solves_two_missing_numbers
    input = [8, 0, 6, 5, 9, 4, 3, 1]
    assert_equal [2, 7], solver.find_remaining_num(input)
  end

  def test_it_finds_remaining_num_if_array_empty
    input = []
    assert_equal [1, 2, 3, 4, 5, 6, 7, 8, 9], solver.find_remaining_num(input)
  end

  def test_solves_missing_numbers_from_two_arrays
    line1 = "8659431\n86594317"
    assert_equal [[2, 7], [2]], solver.find_missing_nums_in_all_arrays(line1)
  end

  def test_solves_missing_numbers_if_one_array_not_missing_anything
    line1 = "8659431\n826594317"
    assert_equal [[2, 7], []], solver.find_missing_nums_in_all_arrays(line1)
  end

  def test_it_returns_nine_numbers_when_an_array_is_empty
    line1 = "8659431\n "
    assert_equal [[2, 7], [1, 2, 3, 4, 5, 6, 7, 8, 9]], solver.find_missing_nums_in_all_arrays(line1)
  end
  ...
  def test_it_can_insert_one_missing_num_by_index
    ...
    solver.insert_missing_num_by_index(arr_of_arrs)
  end
